(() => {
  ...
  function syncNationalMap() {
    ...
    if (trendSvg) {
      ...
      if (line) {
        const vals = nationalProfile.trend.map(x => x.value);
    ...
    const focus = [['KZ63','cre'],['KZ10','esbl'],['KZ75','mrsa']];
    rail.forEach((article,i) => {
      const spec = focus[i]; if (!spec) return;
      const b = article.querySelector('b'); if (b) b.textContent = pct(data.layerValue(spec[0],spec[1]));
      const small = article.querySelector('small'); if (small) small.textContent = regionName(spec[0]);
    });
  }
  ...
  function syncCommand() {
    ...
    paths.forEach((path,index) => {
      ...
      path.setAttribute('fill',commandColor(v));
    ...
    const pcodes = Object.keys(data.regionNames);
    const national = data.layerValue('KZ',layer);
    const ranked = pcodes.map(code => ({pcode:code,value:data.layerValue(code,layer)})).sort((a,b)=>b.value-a.value);
    ...
    const trendSvg = document.querySelector('.command-trend-panel svg');
    if (trendSvg) {
      const line = trendSvg.querySelector('polyline');
      if (line) {
        const vals = profile.trend.map(x=>x.value);
        const min = Math.min(...vals)-2, max = Math.max(...vals)+2, span = Math.max(1,max-min);
        line.setAttribute('points', profile.trend.map((x,i)=>`${(i*700/6).toFixed(1)},${(126-((x.value-min)/span)*102).toFixed(1)}`).join(' '));
      }
    }

    const alerts = [...document.querySelectorAll('.command-alerts article')];
    alerts.forEach((article,i) => {
      const item = ranked[i]; if (!item) return;
      const delta = item.value-national;
      article.dataset.pcode = item.pcode;
      article.dataset.value = item.value;
      article.dataset.delta = delta.toFixed(1);
  ...
  function commandPointer(target) {
    const path = target.closest?.('.command-region[data-pcode]');
    if (!path) return;
    const layer = currentCommandLayer(), v = data.layerValue(path.dataset.pcode,layer);
    const tip = document.getElementById('command-map-tooltip');
    if (tip) tip.innerHTML = `<strong>${regionName(path.dataset.pcode)}</strong>${layer.toUpperCase()} ${pct(v)}<br>DEMO SURVEILLANCE LAYER`;
  }

  function syncIncident(article) {
    if (!article?.dataset.pcode) return;
    const pcode = article.dataset.pcode;
    const layer = article.dataset.layer || currentCommandLayer();
    const p = profileForLayer(pcode,layer);
    const national = data.layerValue('KZ',layer), current = data.layerValue(pcode,layer);
    setTimeout(() => {
  ...
  function clinicalState() {
    ...
    const fromRegional = ctx.source === 'regional-analysis';
    const filterOrg = organismByName[filters[0]?.value || filters[0]?.selectedOptions?.[0]?.textContent || ''];
    const organism = fromRegional && data.organisms[ctx.organism] ? ctx.organism : (filterOrg || ctx.organism || 'eco');
    const materialLabel = (filters[1]?.selectedOptions?.[0]?.textContent || filters[1]?.value || '').toLowerCase();
    const filterMaterial = materialByLabel[materialLabel];
    const material = fromRegional && data.materials[ctx.material] ? ctx.material : (filterMaterial || ctx.material || 'all');
    const periodValue = fromRegional && ctx.period ? ctx.period : (filters[3]?.value || ctx.period || '2026');
    ...
    const filterDrug = drugByLabel[drugLabel];
    const drug = fromRegional && data.organisms[organism]?.drugs?.[ctx.drug] != null ? ctx.drug : (filterDrug || ctx.drug || data.organisms[organism]?.defaultDrug || 'CRO');
  ...
  function syncClinicalMap(state) {
    ...
    if (mapSelect) {
      const opts = data.drugOptions(state.organism);
      const selectedCode = drugByLabel[String(mapSelect.value).toLowerCase()] || state.drug;
      const desired = opts.some(x=>x.code===selectedCode) ? selectedCode : data.organisms[state.organism].defaultDrug;
      const signature = opts.map(x=>x.code).join('|');
      if (mapSelect.dataset.signature !== signature) {
        mapSelect.innerHTML = opts.map(d=>`<option value="${d.names[0]}" data-code="${d.code}">${data.drugName(d.code,lang())}</option>`).join('');
        mapSelect.dataset.signature = signature;
      }
      const option = [...mapSelect.options].find(o=>o.dataset.code===desired); if(option) mapSelect.value=option.value;
      state.drug = desired;
    }
    const paths = [...document.querySelectorAll('.atlas-svg-map .atlas-region[data-pcode]')];
  ...
  function syncClinical() {
    ...
    const org = data.organisms[state.organism] || data.organisms.eco;
    ...
    const pcodes = Object.keys(data.regionNames);
    const ranked = pcodes.map(code=>{const p=data.profile({...state,pcode:code,drug:org.defaultDrug});return{pcode:code,value:p.resistance,delta:p.delta,profile:p}}).sort((a,b)=>b.delta-a.delta);
    const alertCount = ranked.filter(x=>x.delta>=4).length;
    values[4] = [String(alertCount),alertCount?`↑ ${alertCount}`:text('нет новых','жаңа жоқ','no new'),alertCount?text('требуют внимания','назар аударуды қажет етеді','need attention'):text('активных сигналов нет','белсенді сигналдар жоқ','no active signals')];
    stats.forEach((card,i)=>{
      const v=values[i]; if(!v)return;
      const strong=card.querySelector('strong'),small=card.querySelector('small'),em=strong?.querySelector('em');
    ...
    rows.forEach((row,i)=>{
      const item=profiles[i]; row.style.display=item?'':'none'; if(!item)return;
      const name=row.querySelector(':scope > span');if(name)name.textContent=data.drugName(item.code,lang());
      const val=row.querySelector('div > b');if(val)val.textContent=Number(item.p.resistance).toLocaleString(locale(),{minimumFractionDigits:1,maximumFractionDigits:1});
      const bar=row.querySelector('div i u');if(bar){bar.style.width=`${Math.max(3,Math.min(100,item.p.resistance))}%`;bar.className=item.p.resistance>=40?'danger':item.p.resistance>=20?'warn':'safe';}
    ...
    if(trend){
      const selectedLine=trend.querySelector('.country'),nationalLine=trend.querySelector('.world');
      const selectedProfile=data.profile({...state,drug:state.drug}),nationalProfile=data.profile({...state,pcode:'KZ',drug:state.drug});
      const all=[...selectedProfile.trend.map(x=>x.value),...nationalProfile.trend.map(x=>x.value)];
      const min=Math.min(...all)-3,max=Math.max(...all)+3,span=Math.max(1,max-min);
      const points=p=>p.trend.map((x,i)=>`${(i*720/6).toFixed(1)},${(220-((x.value-min)/span)*180).toFixed(1)}`).join(' ');
      if(selectedLine)selectedLine.setAttribute('points',points(selectedProfile));if(nationalLine)nationalLine.setAttribute('points',points(nationalProfile));
    }
    ...
    const heatRows=[...document.querySelectorAll('.heat-table .heat-row')];
    const heatHead=heatRows[0],topDrugs=data.drugOptions(state.organism).slice(0,5);
    if(heatHead)[...heatHead.querySelectorAll('b')].forEach((b,i)=>{b.textContent=topDrugs[i]?.code||'—';});
    const heatPcodes=['KZ71','KZ75','KZ79','KZ35','KZ15'];
    heatRows.slice(1).forEach((row,i)=>{
      const code=heatPcodes[i];const region=row.querySelector('span');if(region)region.textContent=regionName(code);
      [...row.querySelectorAll('b')].forEach((b,j)=>{
        const d=topDrugs[j];if(!d){b.textContent='—';return;}
        const v=data.profile({...state,pcode:code,drug:d.code}).resistance;b.textContent=Math.round(v);b.className=v>=55?'h5':v>=38?'h4':v>=25?'h3':v>=12?'h2':'h1';
      });
    });
    ...
    alerts.forEach((article,i)=>{
      const item=ranked[i];if(!item)return;
      const strong=article.querySelector('strong');if(strong)strong.textContent=`${text('Повышенный R','Жоғары R','Elevated R')}: ${data.drugName(org.defaultDrug,lang())}`;
      const p=article.querySelector('p');if(p)p.textContent=`${regionName(item.pcode)} · ${item.delta>=0?'+':''}${item.delta.toFixed(1)} ${pp()}`;
      const em=article.querySelector('em');if(em)em.textContent=item.delta>=7?text('Высокий','Жоғары','High'):item.delta>=4?text('Средний','Орташа','Medium'):text('Наблюдение','Бақылау','Watch');
    });
  ...
  function clinicalPopup(target) {
    const path=target.closest?.('.atlas-svg-map .atlas-region[data-pcode]');if(!path)return;
    setTimeout(()=>{
      const state=clinicalState();state.pcode=path.dataset.pcode;const p=data.profile(state);
      const popup=document.querySelector('.svg-map-popup');if(!popup?.classList.contains('show'))return;
      const kicker=popup.querySelector('.popup-kicker');if(kicker)kicker.textContent=`${p.organismInfo.short} · ${data.drugName(p.drug,lang())} · ${text('демонстрационные данные','демонстрациялық деректер','demo data')}`;
      const h=popup.querySelector('h3');if(h)h.textContent=regionName(path.dataset.pcode);
      const cells=popup.querySelectorAll('.popup-grid strong');if(cells[0])cells[0].textContent=pct(p.resistance);if(cells[1])cells[1].textContent=num(p.isolates);if(cells[2])cells[2].textContent=String(Math.max(2,Math.round(p.isolates/520)));
      const para=popup.querySelector('p');if(para)para.textContent=`${p.delta>=0?'↑':'↓'} ${Math.abs(p.delta).toFixed(1)} ${pp()} ${text('к национальному демонстрационному уровню','ұлттық демонстрациялық деңгейге','vs national demo baseline')}.`;
    },0);
  }
  ...
  document.addEventListener('click',e=>{
    const languageButton=e.target.closest?.('.atlas-language button');
    if(languageButton){const label=languageButton.textContent.trim().toUpperCase();localStorage.setItem('atlas-preview-language',label==='KZ'?'kk':label==='EN'?'en':'ru');setTimeout(schedule,0);}
  ...
  document.addEventListener('change',e=>{
    if(concept()==='clinical'&&e.target.matches('.filters select,#map-antibiotic')){
      const state=clinicalState();writeContext({...state,source:'clinical-filters'});
    }
    schedule();
  });

  [120,400,900,1600,2600,4200].forEach(delay=>setTimeout(syncAll,delay));
