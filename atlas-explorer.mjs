...
const fmt=v=>v===null?'—':Number(v).toLocaleString(locale(),{maximumFractionDigits:0});
const dec=v=>v===null?'—':Number(v).toLocaleString(locale(),{minimumFractionDigits:1,maximumFractionDigits:1});
...
function writeUrl(push=false){const q=new URLSearchParams(state);q.delete('region');if(state.region!=='KZ')q.set('region',state.region);const url=`${location.pathname}?${q}`;history[push?'pushState':'replaceState']({},'',url);}
function announce(message){document.getElementById('announcer').textContent=message;}
function tone(p){if(p.rate===null)return 'missing';return p.rate<10?'low':p.rate<25?'moderate':p.rate<50?'elevated':'high';}
function color(p){return {low:'#79d9ba',moderate:'#39b9b0',elevated:'#419bc0',high:'#d7b678',missing:'#627c86'}[tone(p)];}
...
function metrics(p){return `<section class="metrics" aria-label="${t('overview')}">${metric(t('tested'),nText(p),t('testedNote'),'lab')}${metric(t('resistance'),pct(p.rate),`${t('ci')}: ${ciText(p)}`,'chart','accent')}${metric(t('laboratories'),fmt(p.labs),t('labsNote'),'globe')}${state.region==='KZ'?metric(t('territories'),'20',t('territoryNote'),'map'):metric(t('testedDrugs'),Object.keys(organisms[state.organism].drugs).length,t('noPooling'),'book')}</section>`;}
function render(){
...
function antibioticTable(){return `<article class="card table-card"><div class="card-heading"><h2>${t('antibiotics')}</h2><p>${t('antibioticsNote')}</p></div><div class="table-scroll"><table><caption class="sr-only">${t('tableCaption')}</caption><thead><tr><th scope="col">${t('drug')}</th><th scope="col">R / N</th><th scope="col">R, %</th><th scope="col">${t('ci')}</th></tr></thead><tbody>${antibioticsFor(state).map(p=>`<tr ${p.drug===state.drug?'class="selected-drug"':''}><th scope="row"><button data-drug="${p.drug}" aria-pressed="${p.drug===state.drug}">${safe(drugName(p.drug,lang))}<small>${p.drug}</small></button></th><td>${p.rate===null?t('suppressed'):`${fmt(p.r)} / ${fmt(p.n)}`}</td><td><b>${pct(p.rate)}</b>${p.rate===null?'':`<span class="inline-bar"><i style="width:${p.rate}%;background:${color(p)}"></i></span>`}</td><td>${ciText(p)}</td></tr>`).join('')}</tbody></table></div></article>`;}
function trendCard(){const data=trendFor(state), max=100, height=145, y=v=>175-v/max*height;const xs=data.map((_,i)=>data.length===1?235:60+i*(350/(data.length-1)));const visible=data.filter(p=>p.rate!==null);const chart=`<svg viewBox="0 0 480 235" role="img" aria-label="${t('trend')} · ${visible.map(p=>`${periodLabel(p.year)} ${pct(p.rate)}`).join(', ')}"><g class="chart-grid">${[0,25,50,75,100].map(n=>`<line x1="40" x2="455" y1="${y(n)}" y2="${y(n)}"/><text x="30" y="${y(n)+3}" text-anchor="end">${n}</text>`).join('')}</g>${data.map((p,i)=>{const prev=data[i-1];return p.rate!==null&&prev?.rate!==null&&prev?`<path class="trend-line ${p.year==='2026'?'partial':''}" d="M${xs[i-1]} ${y(prev.rate)}L${xs[i]} ${y(p.rate)}"/>`:'';}).join('')}${data.map((p,i)=>`<g><text class="chart-year" x="${xs[i]}" y="205" text-anchor="middle">${periodLabel(p.year)}</text>${p.rate===null?'':`<line class="ci-line" x1="${xs[i]}" x2="${xs[i]}" y1="${y(p.ci.low)}" y2="${y(p.ci.high)}"/><circle class="chart-point ${p.year==='2026'?'partial':''}" cx="${xs[i]}" cy="${y(p.rate)}" r="5"/><text class="chart-value" x="${xs[i]}" y="${y(p.rate)-15}" text-anchor="middle">${pct(p.rate)}</text>`}</g>`).join('')}</svg>`;
  return `<article class="card trend-card"><div class="card-heading"><h2>${t('trend')}</h2><p>${t('trendNote')}</p></div>${chart}<div class="trend-values">${data.map(p=>`<div><span>${periodLabel(p.year)}</span><b>${pct(p.rate)}</b><small>R/N: ${p.rate===null?'—':`${fmt(p.r)}/${fmt(p.n)}`}</small><small>${t('ci')}: ${ciText(p)}</small></div>`).join('')}</div>${state.year==='2026'?`<p class="partial-note">${t('trendWarning')}</p>`:''}</article>`;
}
...
function bindMap(){
  const svg=document.getElementById('territory-map');if(!svg)return;
  const bb=svg.querySelector('#territory-shapes').getBBox();const pad=Math.max(bb.width,bb.height)*(state.region==='KZ'?.025:.045);
  viewBox={x:bb.x-pad,y:bb.y-pad,w:bb.width+pad*2,h:bb.height+pad*2};applyZoom();
  svg.addEventListener('pointerover',e=>{const shape=e.target.closest('[data-region]');if(shape&&state.region==='KZ')showTooltip(shape.dataset.region,shape);});
...
function applyZoom(){const svg=document.getElementById('territory-map');if(!svg||!viewBox)return;const {x,y,w,h}=viewBox;svg.setAttribute('viewBox',`${x+w*(1-1/zoom)/2} ${y+h*(1-1/zoom)/2} ${w/zoom} ${h/zoom}`);svg.classList.toggle('zoomed',zoom>1);document.getElementById('pan-hint').hidden=zoom<=1;document.querySelector('[data-zoom="in"]').disabled=zoom>=3;document.querySelector('[data-zoom="out"]').disabled=zoom<=1;}
function showTooltip(code,shape){hideTooltip();shape?.classList.add('hovered');const p=profile({...state,region:code}),tip=document.getElementById('map-tooltip');tip.innerHTML=`<strong>${pname(code)}</strong><span>${pct(p.rate)} R <i>·</i> N ${nText(p)}</span><small>${t('ci')}: ${ciText(p)}</small>`;tip.hidden=false;}
function hideTooltip(){document.querySelectorAll('.territory.hovered').forEach(el=>el.classList.remove('hovered'));const tip=document.getElementById('map-tooltip');if(tip)tip.hidden=true;}
