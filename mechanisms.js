(() => {
  ...
  Object.assign(dict.ru,{genes:'Гены / маркеры · справочные примеры',surveillance_marker:'Фенотипический маркер',evidenceTitle:'Уровень доказательности',evidenceGeneral:'Справочная связь. Фенотипический профиль не подтверждает конкретный ген или механизм; для подтверждения нужен валидированный молекулярный либо специализированный фенотипический тест.',evidenceEsbl:'Обозначения blaTEM и blaSHV без конкретного аллеля не доказывают ESBL-фенотип.',evidenceAmpc:'Хромосомная индуцируемая AmpC и приобретённая плазмидная AmpC требуют раздельной интерпретации.',evidenceFq:'gyrA/parC относятся к изменению мишени, а qnr — к защите мишени; это разные механизмы.',priorityCrab:'WHO BPPL 2024: карбапенем-резистентный A. baumannii · критический приоритет'});
  Object.assign(dict.kk,{genes:'Гендер / маркерлер · анықтамалық мысалдар',surveillance_marker:'Фенотиптік маркер',evidenceTitle:'Дәлел деңгейі',evidenceGeneral:'Анықтамалық байланыс. Фенотиптік профиль нақты генді немесе механизмді растамайды; растау үшін валидтелген молекулалық не арнайы фенотиптік тест қажет.',evidenceEsbl:'Нақты аллель көрсетілмеген blaTEM және blaSHV ESBL фенотипін дәлелдемейді.',evidenceAmpc:'Хромосомалық индукцияланатын AmpC және жүре пайда болған плазмидалық AmpC бөлек түсіндірілуі тиіс.',evidenceFq:'gyrA/parC нысана өзгерісіне, ал qnr нысананы қорғауға жатады; бұлар әртүрлі механизмдер.',priorityCrab:'WHO BPPL 2024: карбапенемге төзімді A. baumannii · критикалық басымдық'});
  Object.assign(dict.en,{genes:'Genes / markers · reference examples',surveillance_marker:'Phenotypic marker',evidenceTitle:'Evidence level',evidenceGeneral:'Reference association. A phenotypic profile does not confirm a specific gene or mechanism; confirmation requires a validated molecular or specialized phenotypic test.',evidenceEsbl:'blaTEM and blaSHV without a specific allele do not establish an ESBL phenotype.',evidenceAmpc:'Chromosomal inducible AmpC and acquired plasmid-mediated AmpC require separate interpretation.',evidenceFq:'gyrA/parC are target alterations, while qnr mediates target protection; these are distinct mechanisms.',priorityCrab:'WHO BPPL 2024: carbapenem-resistant A. baumannii · critical priority'});
  ...
  function priorityLabel(item){return item.code==='aba'&&/critical/i.test(item.priority_tag||'')?t('priorityCrab'):(item.priority_tag||item.code);}
  function organismCards(items){return `<div class="mechanism-links">${(items||[]).map(item=>`<a class="mechanism-link-card" href="./reference.html?tab=organisms&organism=${encodeURIComponent(item.code)}"><strong><i>${esc(localName(item))}</i></strong><small>${esc(priorityLabel(item))}</small><em>${esc(t(item.relevance)||item.relevance||'')}</em></a>`).join('')}</div>`;}
  ...
  function evidenceNote(row){
    const specific=row.code==='ESBL'?t('evidenceEsbl'):row.code==='AMPC'?t('evidenceAmpc'):row.code==='FQ_TARGET'?t('evidenceFq'):'';
    return `<div class="mechanism-evidence"><strong>${esc(t('evidenceTitle'))}: ${esc(t('phenotype_context'))}</strong><p>${esc(t('evidenceGeneral'))}</p>${specific?`<p>${esc(specific)}</p>`:''}</div>`;
  }
