(()=>{
  ...
  function localizeOptions(){
    ...
    if(infection)for(const option of infection.options)setText(option,infectionLabel(option.value));
    const antibiotic=document.getElementById('antibiotic');
    if(antibiotic)for(const option of antibiotic.options)setText(option,drugLabel(option.value));
    const pathogen=document.getElementById('pathogen');
    if(pathogen)for(const option of pathogen.options){const next=replacePhrases(option.textContent);setText(option,next)}
  }
  function localizeCountries(){
    document.querySelectorAll('.country-list [data-pick]').forEach(button=>{
      ...
      const small=span.querySelector('small'),year=small?.textContent||'',name=countryLabel(code);
      const current=[...span.childNodes].find(n=>n.nodeType===Node.TEXT_NODE)?.nodeValue?.trim()||'';
      if(current===name)return;
      span.textContent=name;
      if(year){const s=document.createElement('small');s.textContent=year;span.appendChild(s)}
    });
    document.querySelectorAll('.world-country[data-country]').forEach(path=>{
      ...
      const parts=title.textContent.split(' · '),name=countryLabel(code);if(parts[0]===name)return;parts[0]=name;title.textContent=parts.join(' · ');
    });
    const selected=document.querySelector('.world-country.selected[data-country]'),eyebrow=document.querySelector('#country-panel>.eyebrow');
    if(selected&&eyebrow){const parts=eyebrow.textContent.split(' · '),name=countryLabel(selected.dataset.country);if(parts[0]!==name){parts[0]=name;eyebrow.textContent=parts.join(' · ')}}
  }
  ...
  function apply(){scheduled=false;if(!isRu())return;localizeOptions();localizeCountries();localizeDynamicCopy()}
