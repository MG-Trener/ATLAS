(() => {
  ...
  const copy={
    ru:{launch:'Региональный анализ',eyebrow:'ОРГАНИЗМ × АНТИБИОТИК',demo:'ДЕМОНСТРАЦИОННЫЕ ДАННЫЕ',subtitle:'Детальный региональный профиль AMR',organism:'Микроорганизм',drug:'Антимикробный препарат',material:'Материал',period:'Период',allMaterials:'Все материалы',last12:'Последние 12 месяцев',threeYears:'2024–2026',fullTrend:'2020–2026',resistant:'Резистентные',susceptible:'Чувствительные',intermediate:'Промежуточные',isolates:'Изоляты',mdr:'MDR',trend:'Динамика резистентности',trendSub:'Доля R по годам, %',materials:'Материалы',materialsSub:'Доля R и число изолятов',compare:'к уровню страны',national:'Казахстан',sample:'Оценка выборки',phenotypes:'AMR-фенотипы',good:'Выборка достаточна для демонстрационного аналитического профиля.',moderate:'Умеренная выборка: интерпретируйте различия с осторожностью.',low:'Малая выборка: показатель нельзя использовать для устойчивых выводов.',organismRef:'Открыть организм в справочнике',drugRef:'Открыть препарат в справочнике',mechanismRef:'Открыть механизм AMR',close:'Закрыть'},
    kk:{launch:'Өңірлік талдау',eyebrow:'МИКРООРГАНИЗМ × ПРЕПАРАТ',demo:'ДЕМОНСТРАЦИЯЛЫҚ ДЕРЕКТЕР',subtitle:'AMR өңірлік егжей-тегжейлі профилі',organism:'Микроорганизм',drug:'Микробқа қарсы препарат',material:'Материал',period:'Кезең',allMaterials:'Барлық материалдар',last12:'Соңғы 12 ай',threeYears:'2024–2026',fullTrend:'2020–2026',resistant:'Төзімді',susceptible:'Сезімтал',intermediate:'Аралық',isolates:'Изоляттар',mdr:'MDR',trend:'Төзімділік динамикасы',trendSub:'Жылдар бойынша R үлесі, %',materials:'Материалдар',materialsSub:'R үлесі және изоляттар саны',compare:'ел деңгейімен',national:'Қазақстан',sample:'Іріктеме бағасы',phenotypes:'AMR фенотиптері',good:'Демонстрациялық аналитикалық профиль үшін іріктеме жеткілікті.',moderate:'Орташа іріктеме: айырмашылықтарды сақтықпен түсіндіріңіз.',low:'Іріктеме аз: көрсеткіш тұрақты қорытынды үшін жеткіліксіз.',organismRef:'Микроорганизм анықтамалығы',drugRef:'Препарат анықтамалығы',mechanismRef:'AMR механизмін ашу',close:'Жабу'},
    en:{launch:'Regional analysis',eyebrow:'ORGANISM × ANTIMICROBIAL',demo:'DEMO DATA',subtitle:'Detailed regional AMR profile',organism:'Organism',drug:'Antimicrobial',material:'Specimen',period:'Period',allMaterials:'All specimens',last12:'Last 12 months',threeYears:'2024–2026',fullTrend:'2020–2026',resistant:'Resistant',susceptible:'Susceptible',intermediate:'Intermediate',isolates:'Isolates',mdr:'MDR',trend:'Resistance trend',trendSub:'R proportion by year, %',materials:'Specimens',materialsSub:'R proportion and isolate count',compare:'vs country',national:'Kazakhstan',sample:'Sample assessment',phenotypes:'AMR phenotypes',good:'Sample size is sufficient for the demo analytical profile.',moderate:'Moderate sample size: interpret differences cautiously.',low:'Small sample: the indicator is insufficient for stable conclusions.',organismRef:'Open organism reference',drugRef:'Open antimicrobial reference',mechanismRef:'Open AMR mechanism',close:'Close'}
  };
  ...
  function populateControls(){
    ...
    periodSelect.innerHTML=`<option value="2026">2026</option><option value="last12">${esc(t('last12'))}</option><option value="2024-2026">${esc(t('threeYears'))}</option><option value="2020-2026">${esc(t('fullTrend'))}</option>`;periodSelect.value=state.period;
  }
  ...
  function render(){
    ...
    drawer.querySelector('#ra-kpis').innerHTML=`<article class="r"><span>${t('resistant')}</span><strong>${fmt(p.resistance)}%</strong><small>R</small></article><article class="s"><span>${t('susceptible')}</span><strong>${fmt(p.susceptible)}%</strong><small>S</small></article><article class="i"><span>${t('intermediate')}</span><strong>${fmt(p.intermediate)}%</strong><small>I</small></article><article><span>${t('isolates')}</span><strong>${p.isolates.toLocaleString(locale())}</strong><small>N</small></article><article><span>${t('mdr')}</span><strong>${fmt(p.mdr)}%</strong><small>MDR</small></article>`;
    ...
    const warning=drawer.querySelector('#ra-warning');warning.className=`regional-analysis-warning ${q==='good'?'good':''}`;warning.innerHTML=`<strong>${esc(t('sample'))}:</strong> ${esc(t(q))}`;
