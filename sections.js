(() => {
  ...
  style.href = './sections.css?v=20260917-1';
  ...
  function renderOrganisms() {
    ...
    const draw = () => {
      ...
      list.innerHTML = `<div class="catalog-head"><span>Микроорганизм</span><span>Изоляты</span><span>%R маркерной пары</span><span>Тренд</span><span>Фенотип · предполагаемый</span><span></span></div>` + rows.map((o) => `
  ...
  function renderAntibiotics() {
    view.innerHTML = sectionHead('Антибиотики', 'Профиль тестирования и резистентности по антибактериальным препаратам.', '<button class="btn secondary">Справочник EUCAST</button>') + `
      <div class="metric-strip"><div><span>Препаратов в каталоге</span><strong>42</strong></div><div><span>AST результатов</span><strong>628 410</strong></div><div><span>Интерпретируемых AST</span><strong>94,1%</strong></div><div><span>Сигналов на проверке</span><strong>7</strong></div></div>
  ...
  function renderAnalytics() {
    view.innerHTML = sectionHead('Аналитика', 'Конструктор исследовательского среза и обзор ключевых тенденций.') + `
      <div class="analytics-filters"><label>Организм<select><option>E. coli</option><option>K. pneumoniae</option><option>S. aureus</option></select></label><label>Период<select><option>2022–2026 YTD</option><option>2026 YTD</option></select></label><label>Регион<select><option>Казахстан</option><option>Астана</option><option>Алматы</option></select></label><label>Материал<select><option>Все материалы</option><option>Моча</option><option>Кровь</option></select></label></div>
      <div class="analytics-grid"><article class="analysis-card wide"><div class="mini-head"><div><h3>Изменение резистентности</h3><p>Цефтриаксон · E. coli · R/N tested</p></div><b>+9,1 п.п.</b></div><svg class="analysis-chart" viewBox="0 0 700 230" preserveAspectRatio="none"><g class="grid-lines"><line x1="0" y1="40" x2="700" y2="40"/><line x1="0" y1="90" x2="700" y2="90"/><line x1="0" y1="140" x2="700" y2="140"/><line x1="0" y1="190" x2="700" y2="190"/></g><polyline points="0,178 140,164 280,141 420,122 560,91 700,67"/></svg><div class="year-axis"><span>2021</span><span>2022</span><span>2023</span><span>2024</span><span>2025</span><span>2026 YTD</span></div></article><article class="analysis-card"><h3>Фенотипы · предполагаемые</h3><div class="donut"><div><strong>18,4%</strong><span>ESBL</span></div></div><div class="rank-list"><span><b>ESBL</b><em>demo v0.1</em></span><span><b>MDR</b><em>demo v0.1</em></span><span><b>Carb-R</b><em>фенотип</em></span></div></article><article class="analysis-card"><h3>Качество выборки</h3><div class="quality-score">A</div><p>Высокая полнота AST и достаточная выборка для демонстрационного анализа.</p><div class="quality-mini"><span>N tested <b>84 215</b></span><span>Регионов <b>20</b></span><span>Лабораторий <b>86</b></span></div></article></div>`;
  }

  function renderComparison() {
    view.innerHTML = sectionHead('Сравнение', 'Параллельное описательное сравнение двух регионов или выборок.') + `
      ...
      <div class="compare-grid"><article><span>N tested</span><strong>12 842</strong><em>Астана</em></article><article><span>N tested</span><strong>14 516</strong><em>Алматы</em></article><article><span>%R · E. coli × CRO</span><strong>27,8%</strong><em>95% ДИ 27,0–28,6</em></article><article><span>%R · E. coli × CRO</span><strong>32,6%</strong><em>95% ДИ 31,8–33,4</em></article></div>
      <div class="method-note"><strong>Важно:</strong> территории не ранжируются. Сравнение допустимо только для одинаковой пары организм × препарат, периода, стандарта breakpoint и сопоставимого лабораторного покрытия.</div>
  ...
  function renderSignals() {
    view.innerHTML = sectionHead('AMR Radar', 'Демонстрационные сигналы, которые требуют аналитической проверки до эскалации.') + `
      ...
      <div class="method-note"><strong>Статус:</strong> алгоритм demo v0.1 · baseline 2022–2025 · минимум N tested 30 · сигналы не являются подтверждённой вспышкой или молекулярным механизмом.</div>
  ...
  function renderPublications() {
    ...
    view.innerHTML = sectionHead('Публикации', 'Макеты будущих отчётов и методических материалов; публикация отключена до научного утверждения.') + `<div class="publication-grid">${pubs.map((p,i)=>`<article><div class="pub-type">${p[1]} · МАКЕТ</div><h3>${p[0]}</h3><p>Демонстрационная карточка будущего научного материала. Не является опубликованной статистикой.</p><div><span>${p[2]}</span><span>${p[3]}</span></div></article>`).join('')}</div>`;
  }
