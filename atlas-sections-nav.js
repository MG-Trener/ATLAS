(()=>{
  'use strict';
  const labels={
    ru:{home:'Казахстан',world:'Мир / WHO GLASS',reference:'Справочник',mechanisms:'Механизмы',glossary:'Словарь AMR',methodology:'Методология',brand:'Антимикробная резистентность'},
    kk:{home:'Қазақстан',world:'Әлем / WHO GLASS',reference:'Анықтамалық',mechanisms:'Механизмдер',glossary:'AMR сөздігі',methodology:'Әдістеме',brand:'Антимикробтық төзімділік'},
    en:{home:'Kazakhstan',world:'World / WHO GLASS',reference:'Reference',mechanisms:'Mechanisms',glossary:'AMR glossary',methodology:'Methodology',brand:'Antimicrobial resistance'}
  };
  ...
  function loadCss(){if(document.querySelector('link[data-atlas-unified]'))return;const l=document.createElement('link');l.rel='stylesheet';l.href='./atlas-unified.css?v=20260921-1';l.dataset.atlasUnified='1';document.head.appendChild(l)}
  ...
  function makeHeader(){const l=labels[lang()]||labels.ru;const h=document.createElement('header');h.className='site-header';h.dataset.atlasUnifiedHeader='1';h.innerHTML=`<a class="brand" href="./index.html"><img src="./atlas-mark.svg" width="40" height="40" alt=""><span><b>AMR<span>Atlas</span></b><small>${l.brand}</small></span></a><nav aria-label="AMR Atlas">${navHtml()}</nav>${languageHtml()}`;document.body.prepend(h);return h}
  ...
  function normalizeHeader(){
    ...
    h.dataset.atlasUnifiedHeader='1';const l=labels[lang()]||labels.ru;const brand=h.querySelector('.brand');if(brand){brand.href='./index.html';const small=brand.querySelector('small');if(small&&small.textContent!==l.brand)small.textContent=l.brand}
