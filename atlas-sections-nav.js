(()=>{
  ...
  function ensureFavicons(){
    ...
    iconLink.type='image/svg+xml';iconLink.href=href;iconLink.dataset.atlasFavicon='1';
    document.querySelectorAll('link[rel="apple-touch-icon"]').forEach(link=>link.remove());
  }
  function brandHtml(){const l=labels[lang()]||labels.ru;return `<img class="brand-symbol" src="./assets/branding/logo.png" width="48" height="48" decoding="async" fetchpriority="low" alt=""><span class="brand-copy"><b>AMR <span>Atlas</span></b><small>${l.tagline}</small></span>`}
  ...
  function normalizeHeader(){
    ...
    brand.href='./index.html';const signature=`${lang()}|${currentPage()}`;if(brand.dataset.atlasBrandSignature!==signature){brand.innerHTML=brandHtml();brand.dataset.atlasBrandSignature=signature}
    ...
    const html=navHtml();if(nav.dataset.atlasNavSignature!==signature){nav.innerHTML=html;nav.dataset.atlasNavSignature=signature}
